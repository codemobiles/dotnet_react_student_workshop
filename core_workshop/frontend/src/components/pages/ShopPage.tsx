...
const Shop = () => {
  ...
  const isSelectedItem = (product: Product) => {
    const index = shopReducer.mOrderLines.findIndex((item: Product) => {
  ...
  const renderOrderRows = () => {
    ...
    return mOrderLines.map((item: Product) => (
      <ListItem button divider className="h-[100px]" key={item.productId}>
